...
```{r}
library(readr)
datos <- read_csv("C://Users//jc124//OneDrive//Documents//EleccionesMagistrados//01.csv")
View(datos)
```
```{r}
datos <- datos %>%
...
```
```{r}
duplicados <- todas_juntas %>%
  group_by(`Persona Candidata`) %>%
  summarise(cantidad = n()) %>%
  filter(cantidad > 1)

cat("Personas que se postulan a más de un poder:", nrow(duplicados), "\n")
print(duplicados)
